...
rounded_length = round(wire_length, 4);
rounded_input = round(nom_length, 4);
...
if direction == "Left"
    while rounded_length > rounded_input %while the line length is too long, remove points
        modified_points(:,1) = []; 
        rounded_length = round(arclength(modified_points(1,:), modified_points(2,:), modified_points(3,:)), 5);
        p = p + 1;
        if (p > 100)
            break;
        end
